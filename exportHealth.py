...
parser = argparse.ArgumentParser() #Neuen Parser erstellen. Durch diesen wird das CLI ermöglicht.
group = parser.add_mutually_exclusive_group() #Neue Gruppe für Befehle erstellen. Diese können nicht zusammen genutzt werden.
group.add_argument("-xc", "--tcx2csv", help="Exports your .tcx Data in a .csv File.", action="store_true") #Hinzufügen von den einzelnen Flags.
...
if args.csvgpslist: #Fallunterscheidungen für die einzelnen Inputs. Falls -cl ausgewählt wird, wird getLatLongInCsv ausgeführt.
    ...
    getTCXDataToCSVGFit(args.pathToDir)
